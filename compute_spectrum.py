...
def compute_spectrum(net=None, list_T=None, lamda=None, t_start=None, t_stop=None,
                                    verbose=False,
    ...
    if force_csr:
        ...
        try:
            ...
            spectrum[f'{lamda:.11f}'] = [eigs2]
            del T
        except ValueError:
            spectrum[f'{lamda:.11f}'] = [np.zeros(T.shape[0])]
    else:
        raise Exception
    ...
    for k in k_range:
        ...
        try:
            ...
            eigs2 = np.where(eigs > 0, eigs, 0)
            eigs2 = eigs2 / eigs2.sum()
            spectrum[f'{lamda:.11f}'].append(eigs2)
            del T
        except ValueError:
            spectrum[f'{lamda:.11f}'].append(spectrum[f'{lamda:.11f}'][-1])
    t_end = time.time()-t0
