...
def compute_vonNeumann_entropy(net=None, list_T=None, lamda=None, t_start=None, t_stop=None,
                                    verbose=False,
    ...
    if force_csr:
        ...
        try:
            ...
            vNS[f'{lamda:.11f}'] = [- 1/np.log(net.num_nodes) * np.trace(rhologrho)]
            del T
            del rho
            del rhologrho
        except ValueError:
    ...
    for k in k_range:
        if verbose and not k%1000:
            print('PID ', os.getpid(), ' : ',k, ' over ' , len(net.inter_T[lamda]))
        ...
        try:
            rhologrho = rho @ d_logm(rho)
            vNS[f'{lamda:.11f}'].append(- 1/np.log(net.num_nodes) * np.trace(rhologrho))
            del T
            del rho
            del rhologrho
        except ValueError:
            vNS[f'{lamda:.11f}'].append(vNS[f'{lamda:.11f}'][-1])
    t_end = time.time()-t0
...
def compute_spectral_vonNeumann_entropy(net=None, list_T=None, lamda=None, t_start=None, t_stop=None,
                                    verbose=False,
                                    # save_intermediate=True,
                                    reverse_time=False,
                                    force_csr=True,
    ...
    if force_csr:
        ...
        T = T.toarray()
        try:
            eigs = np.linalg.eigvals(T) # this is for numpy arrays and it returns all the eigenvalues
            eigs2 = np.where(eigs > 0, eigs, 0)
            eigs2 = eigs2 / eigs2.sum()
            eigs2 = np.where(eigs2 > 0, eigs2, 1)
            normalizazion_constant = net.num_nodes #np.sum(eigs < 1)
            spectral_vNS[f'{lamda:.11f}'] = [- 1/np.log(normalizazion_constant) * np.sum(eigs2*np.log(eigs2))]
            del T
        except ValueError:
            spectral_vNS[f'{lamda:.11f}'] = [100]
    else:
        raise Exception
    ...
    for k in k_range:
        ...
        try:
            ...
            eigs2 = eigs2 / eigs2.sum()
            eigs2 = np.where(eigs2 > 0, eigs2, 1)
            normalizazion_constant = net.num_nodes
            spectral_vNS[f'{lamda:.11f}'].append(- 1/np.log(normalizazion_constant) * np.sum(eigs2*np.log(eigs2)))
            del T
        except ValueError:
            spectral_vNS[f'{lamda:.11f}'].append(spectral_vNS[f'{lamda:.11f}'][-1])
    t_end = time.time()-t0
