...
def worker(lamda):
    with open(folder + f'window_T_selected/50/window_T{lamda:.11f}', 'rb') as f:
        dict_T = pickle.load(f)

    
    try:
        # S = compute_S_rate.compute_entropy_rate(net=net, list_T=dict_T['T'],
        #                             lamda=lamda, force_csr=True,
        #                             time_domain= list(np.arange(0, len(net.times)-2))) #[len(net.times)-2]) #list(np.arange(0, len(net.times), 10)))
        S = compute_S_rate.compute_conditional_entropy(net=net, list_T=dict_T['window_T'],
                                    lamda=lamda, force_csr=True,
                                    time_domain= list(np.arange(0,len(net.times) -50 -1))) #list(np.arange(0,len(net.times) -1556 -180 -1))) #list(np.arange(0, len(net.times)-2))) #[len(net.times)-2]) #list(np.arange(0, len(net.times), 10)))
        
        # S = compute_S_rate.compute_instantaneous_entropy_rate(net=net, list_T=dict_T['T'], list_L=dict_L['L'],
        #                             lamda=lamda, force_csr=True,
        #                             time_domain= list(np.arange(0, len(net.times)-2))) #[len(net.times)-2]) #list(np.arange(0, len(net.times), 10)))
        
        S_rate={'lamda': f'{lamda:.11f}', 'window_S': S}
        file= folder + f'window_S_selected/50/window_S{lamda:.11f}'
