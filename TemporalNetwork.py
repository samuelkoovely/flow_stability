    ...
    def compute_entropy(self, lamda=None, t_start=None, t_stop=None,
        ...
        if lamda not in self.S.keys():
            ...
            if force_csr:
                ...
                #logT = self.T[lamda][k_init].tocsr().log1p()
                TlogT = csr_matrix((T.data*np.log(T.data), T.indices, T.indptr), shape=T.shape)
                # TlogT = csr_matrix(self.T[lamda][k_init].tocsr(), logT)
                self.S[lamda] = [-np.sum(p @ TlogT, where= np.isfinite(p @ TlogT))]
            else:
                raise Exception("Use force_csr=True")
            
            if tol is not None:
                set_to_zeroes(self.S[lamda][0],tol)
                # inplace_csr_row_normalize(self.T[lamda][0])
            '''else:
                if force_csr:
                    self.T[lamda] = self.inter_T[lamda][k_init].tocsr()
            ...
            for k in k_range:
                ...
                p = p0 @ self.T[lamda][k]
                #logT = np.log(self.T[lamda][k])

                #logT = self.T[lamda][k].log1p()
                T = self.T[lamda][k].tocsr()
                TlogT = csr_matrix((T.data*np.log(T.data), T.indices, T.indptr), shape=T.shape)
                    
                    
                self.S[lamda].append(-np.sum(p @ TlogT, where= np.isfinite(p @ TlogT)))
