...
class ApplyNodeFunc(nn.Module):
    ...
    def __init__(self, mlp):
        ...
        self.mlp = mlp
        #self.bn = nn.BatchNorm1d(self.mlp.output_dim)  change suggested by chatgpt to avoir batch_size issue
        #self.bn = nn.LayerNorm(self.mlp.output_dim)
        self.norm = nn.LayerNorm(self.mlp.output_dim)
    ...
    def __init__(self, num_layers, input_dim, hidden_dim, output_dim, dropout=0.1):
        ...
        if num_layers < 1:
            ...
            for layer in range(num_layers - 1):
                #self.batch_norms.append(nn.BatchNorm1d((hidden_dim))) change suggested by chatgpt for batch_size issue
    ...
    def forward(self, x):
        if self.linear_or_not:
            ...
            for i in range(self.num_layers - 1):
                # h = F.relu(self.batch_norms[i](self.linears[i](h))) change suggested by chatgpt for batch_size issue
                h = F.relu(self.batch_norms[i](self.linears[i](h)))
                h = self.dropout(h)
                #print("MLP input:", h.shape)
