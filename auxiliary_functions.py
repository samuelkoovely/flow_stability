...
def filter_signal(S, dt, k):
    """
    Filters the signal by keeping the k highest-magnitude frequency components.
    
    Parameters:
    S  : numpy array, the input signal
    dt : float, time step between samples
    k  : int, number of dominant frequencies to retain
    
    Returns:
    reconstructed_signal : numpy array, the filtered signal
    freqs : numpy array, the corresponding frequency components
    fft_vals_filtered : numpy array, the filtered FFT values
    """
    ...
    # Find indices of the k highest-magnitude frequencies
    dominant_indices = np.argsort(magnitude)[-k:]  # Get indices of k largest values
    
    # Create a filtered FFT array: keep only the top k frequencies, zero out the rest
    fft_vals_filtered = np.zeros_like(fft_vals)
    fft_vals_filtered[dominant_indices] = fft_vals[dominant_indices]
    
    # Compute inverse FFT
    reconstructed_signal = ifft(fft_vals_filtered).real  # Taking the real part
    
    return reconstructed_signal, fftfreq(N, d=dt), fft_vals_filtered

import numpy as np
import matplotlib.pyplot as plt
from scipy.fft import fft, ifft, fftfreq

def extract_frequencies(S, dt, indices):
    """
    Extracts specific frequency components from the FFT of a signal and reconstructs them individually.
    
    Parameters:
    S       : numpy array, the input signal
    dt      : float, time step between samples
    indices : list of int, indices of the frequencies to extract (sorted by magnitude)
    
    Returns:
    reconstructed_signals : dict, mapping of frequency index to its reconstructed time-domain signal
    freqs : numpy array, corresponding frequency components
    """
    N = len(S)  # Number of samples
    ...
    # Dictionary to store reconstructed signals for each selected frequency index
    reconstructed_signals = {}

    for idx in selected_indices:
        # Create an FFT array with only the selected frequency
        fft_component = np.zeros_like(fft_vals, dtype=complex)
        fft_component[idx] = fft_vals[idx]  # Keep only this frequency

        # Ensure symmetry by keeping its conjugate counterpart
        if idx != 0 and idx < N // 2:  # Avoid double-counting DC component
            fft_component[-idx] = fft_vals[-idx]  # Conjugate pair

        # Inverse FFT to reconstruct time-domain signal
        reconstructed_signal = ifft(fft_component).real
        reconstructed_signals[idx] = reconstructed_signal  # Store it in the dictionary

    return reconstructed_signals, freqs